    ...
    <div class="row">
        ...
        <div class="col-sm-12 col-md-4">
            <?= $form->field($node, 'url_type')->dropDownList([
                ...
                'leadership-category' => "Rahbariyat turlari",
                'other' => "Boshqa",
                'new-category' => 'Yangilik kategoriyasi',
                'page' => 'Sahifalar',
                'video-category'=>'Videolar',
                   ...
                   var t =  $(this).val()
                    ...
                    else if (t === "leadership-category" || t == "c-action" || t=="new-category"||t=="page" ||t =="video-category"){
                        $.ajax({
                            url: menu_url,
                            data: {type:t},
                            type: "get",
        ...
        <div class="col-sm-12 col-md-8">
            <div class="form-group highlight-addon field-tree-url_value">
                ...
                <div id="menu-value">
                    ...
                    elseif ($node->url_type == 'leadership-category')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(LeadershipCategory::find()->all(), 'id', 'name'))->label(false);
                    elseif ($node->url_type == 'new-category')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(PostCategory::find()->all(), 'id', 'name'))->label(false);
                    elseif ($node->url_type == 'page')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(Page::find()->andWhere(['status' => Page::STATUS_ACTIVE])->all(), 'id', 'title'))->label(false);
                    elseif ($node->url_type == 'video-category')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(VideoCategory::find()->andWhere(['status' => VideoCategory::STATUS_ACTIVE])->all(), 'id', 'title'))->label(false);
                    else
